...
*** note
This applies to commits in Chromium OS repositories. For changes to Chromium
...
SIE on the project, rather than the Chrome OS TPM managing the release branch.
...
Otherwise, if there is a conflict then the merge must be done manually.
Cherry-picking via UI will fail at step 4 above, and the UI will notify you of
the conflict, before proceeding to a CL on the target-branch that has the
unresolved conflicts.  Gerrit currently provides no way of dealing with merge
conflicts in the UI.
...
top-of-tree (ToT) in Chrome OS is using `cros_merge_to_branch`.
...
described in the [Chromium OS Developer Guide][developer-guide] i.e.
...
# and the branch to merge to is the R76 branch on the Chrome OS server:
